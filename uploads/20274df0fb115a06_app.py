...
def dashboard_context(analysis: dict | None) -> dict:
    if not analysis:
        return {"processes": [], "iocs": [], "timeline": [], "clean_count": 0, "suspicious_count": 0, "malicious_count": 0, "analysis": None, "analysis_id": None}

    processes = analysis.get("memory_analysis", {}).get("processes", [])
    iocs = analysis.get("iocs", [])
    timeline = analysis.get("timeline", [])
    clean_count = sum(1 for proc in processes if proc.get("status") == "Clean")
    suspicious_count = sum(1 for proc in processes if proc.get("status") == "Suspicious")
    malicious_count = 1 if analysis.get("malware_present") else 0

    return {"processes": processes, "iocs": iocs, "timeline": timeline, "clean_count": clean_count, "suspicious_count": suspicious_count, "malicious_count": malicious_count, "analysis": analysis, "analysis_id": analysis.get("analysis_id")}
...
def run_analysis_job(job_id: str, stored_path: str, filename: str) -> None:
    ...
    try:
        ...
        analysis = analyze_file(stored_path, volatility_root=os.getenv("VOLATILITY3_PATH"), progress_callback=progress_callback)
